  ...
  export default {
        ...
        methods: {
            ...
            async login(){
                ...
                try {
                    let res1 = await post('/account/sign-in',{
                        username: this.formItem.account,
                        password: this.formItem.password
                    })
                    this.$Message.success("登录成功");
                    localStorage.setItem('csrf-token', res1.data);
                    this.$store.commit('setToken', res1.data);
                    let res2 = await post('/account/current')
                    this.$store.commit('setUserId', res2.data.id);
                    this.$store.commit('setUserName', res2.data.username);
